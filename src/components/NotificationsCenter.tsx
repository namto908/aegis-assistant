...
import { 
  Bell, BellOff, Check, Trash2, ShieldAlert, CheckSquare, Server, 
  Terminal, Sparkles, Inbox, MessageSquare, Newspaper, Clock, 
  ExternalLink, X, ChevronRight, Calendar, AlertCircle, RefreshCw 
} from "lucide-react";
import { Notification, ThemeColor, Task, ServerStatus } from "../types";
...
export default function NotificationsCenter({ 
  ...
  const [topic, setTopic] = useState<"hacker-news" | "ai-news" | "system-summary" | "custom">("hacker-news");
  ...
  const triggerNewsPush = async (signal?: AbortSignal) => {
    setLoading(true);
    setLoadingLogs(["Kích hoạt tác vụ điểm tin sáng..."]);
    
    setTimeout(() => {
      setLoadingLogs((prev) => [...prev, "Khởi động phiên kết nối Gemini AI..."]);
    }, 600);

    setTimeout(() => {
      setLoadingLogs((prev) => [...prev, topic === "system-summary" ? "Thu thập dữ liệu trạng thái hệ thống Aegis..." : "Đang thực hiện Google Search Grounding..."]);
    }, 1200);

    try {
      const apiBase = apiBaseUrl ? apiBaseUrl.replace(/\/$/, "") : "";
      const response = await fetch(`${apiBase}/api/gemini/news`, {
        method: "POST",
      ...
      if (!response.ok || data.error) {
        throw new Error(data.error || "Lỗi phản hồi từ máy chủ tổng hợp tin tức.");
      }
      
      setTimeout(() => {
        setLoadingLogs((prev) => [...prev, "Đang xử lý, tóm tắt và biên tập bài viết..."]);
      }, 1800);
      ...
      setTimeout(() => {
        ...
        setSelectedNews(newNotif); // Automatically open the news article detail panel
        setLoading(false);
        setLoadingLogs([]);
      }, 2400);
      ...
      setLoadingLogs((prev) => [...prev, `❌ Lỗi: ${err.message || "Không thể tổng hợp tin tức lúc này. Vui lòng kiểm tra API key hoặc cấu hình server."}`]);
      setTimeout(() => {
        setLoading(false);
      }, 2000);
    }
  ...
  return (
    <div className="space-y-4 pb-4 relative h-full flex flex-col min-h-0">
      {/* Header */}
      <div className="flex items-center justify-between pb-2 border-b border-white/5">
        <div>
          <h1 className="text-xl font-bold text-white font-display">Trung tâm thông báo</h1>
          <p className="text-xs text-slate-400">Xem các sự kiện, cảnh báo và tin nhắn trợ lý</p>
        </div>
      ...
      {/* 7:00 AM News Hub Scheduler Card */}
      <div className="glass-card rounded-2xl p-4 border border-white/10 bg-slate-900/40 relative overflow-hidden space-y-3">
        {/* Glow accent */}
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="text-xs font-bold text-white font-display">Lập lịch Bản tin 7:00 Sáng</h3>
              <p className="text-[10px] text-slate-400">Tự động hóa điểm tin qua Cron-job</p>
            </div>
          </div>
          
          {/* Cron Trigger Status Toggle */}
          <button 
            onClick={() => setIsCronEnabled(!isCronEnabled)}
            className={`text-[10px] px-2 py-0.5 rounded-full border flex items-center gap-1 cursor-pointer transition ${
              isCronEnabled 
          ...
          >
            <Clock size={10} />
            <span>{isCronEnabled ? "Đang lên lịch" : "Tắt lịch"}</span>
          </button>
        </div>

        {/* Info label */}
        <div className="flex items-center gap-1.5 text-[10px] text-slate-400 bg-slate-950/30 p-2 rounded-xl border border-white/5 font-mono">
          <span className={`w-1.5 h-1.5 rounded-full ${isCronEnabled ? "bg-emerald-400 animate-pulse" : "bg-slate-500"}`}></span>
          <span>{isCronEnabled ? "● Lịch chạy: 07:00 Sáng mỗi ngày (Hằng ngày)" : "● Lập lịch tự động đang bị tạm dừng"}</span>
        </div>

        {/* Source selector */}
        <div className="space-y-1.5">
          <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider font-display">Chủ đề điểm tin</label>
          <div className="grid grid-cols-2 gap-1.5">
            {[
              { id: "hacker-news", label: "Hacker News", desc: "Tin công nghệ" },
              { id: "ai-news", label: "Tin tức AI", desc: "LLM & Trend" },
              { id: "system-summary", label: "Aegis System", desc: "Hệ thống" },
              { id: "custom", label: "Chủ đề tùy chỉnh", desc: "Nhập thủ công" }
            ].map((s) => (
              <button
                key={s.id}
                disabled={loading}
        ...
        {topic === "custom" && (
          <div className="space-y-1.5 animate-in fade-in slide-in-from-top-1 duration-155">
            <label className="text-[10px] font-bold text-slate-400 uppercase tracking-wider font-display">Nội dung tóm tắt bạn muốn</label>
            <input 
              type="text" 
              value={customTopic}
              onChange={(e) => setCustomTopic(e.target.value)}
              placeholder="Ví dụ: Tin nóng Bitcoin, Bóng đá Ngoại Hạng Anh sáng nay..."
              className={`w-full bg-slate-950/50 border border-white/10 rounded-xl px-3 py-2 text-xs text-white placeholder-slate-500 focus:outline-none focus:${theme.border} transition font-sans`}
              disabled={loading}
            />
          </div>
        )}
        ...
        {loading ? (
          <div className="space-y-2 p-2.5 bg-slate-950/40 rounded-xl border border-white/5">
            <div className={`flex items-center gap-2 text-xs ${theme.text}`}>
              <RefreshCw size={12} className={`animate-spin ${theme.text}`} />
              <span>Đang biên soạn Bản tin sáng...</span>
            </div>
            <div className="space-y-1 pl-4 font-mono text-[9px] text-slate-500 border-l border-white/5">
          ...
          <button
            ...
            className={`w-full py-2 ${theme.bgMuted} hover:bg-white/10 ${theme.text} hover:text-white rounded-xl transition text-xs font-bold border ${theme.borderMuted} flex items-center justify-center gap-1.5 cursor-pointer shadow-lg`}
          >
            <Sparkles size={13} className="animate-pulse" />
            <span>⚡ Đẩy bản tin 7:00 Sáng ngay lập tức</span>
          </button>
        )}
      </div>
        ...
        {filteredNotifs.length === 0 ? (
          <div className="glass-card rounded-2xl p-10 text-center space-y-3">
            ...
            <div className="space-y-1">
              ...
              <p className="text-xs text-slate-500">Khi trợ lý kích hoạt sự kiện hoặc tác vụ mới, thông tin sẽ xuất hiện tại đây.</p>
            </div>
          </div>
        ) : (
              ...
              return (
                <motion.div
                  ...
                  className={`glass-card rounded-2xl p-3.5 border-l-4 transition-all duration-200 cursor-pointer ${
                    notif.read 
                      ? "border-l-slate-700 bg-slate-950/20 opacity-35 hover:opacity-55 scale-[0.99] border-t-transparent border-r-transparent border-b-transparent" 
      ...
      {/* Suggestion box */}
      <div className={`glass-card rounded-2xl p-4 bg-white/5 border ${theme.borderMuted} mt-4 flex items-start gap-3`}>
        <div className={`p-2 ${theme.bgMuted} ${theme.text} rounded-full border ${theme.borderMuted} flex-shrink-0`}>
          <Bell size={16} />
        </div>
        <div>
          <h4 className={`text-xs font-bold ${theme.textMuted} uppercase tracking-wider font-display`}>Gợi ý từ Trợ Lý</h4>
          <p className="text-xs text-slate-300 leading-relaxed mt-1">
            "Bạn có thể đổi chủ đề điểm tin sang Tin tức AI hoặc Hacker News để cập nhật xu thế công nghệ mới nhất vào lúc 07:00 sáng mỗi ngày."
          </p>
        </div>
      </div>
      ...
      <AnimatePresence>
        {selectedNews && (
          <motion.div 
            ...
            className="absolute inset-0 bg-slate-950/98 backdrop-blur-2xl z-50 flex flex-col p-5 overflow-y-auto no-scrollbar rounded-t-[32px] border-t border-white/10 shadow-[0_-10px_30px_rgba(0,0,0,0.8)]"
          >
            {/* Header */}
            <div className="flex items-center justify-between pb-3 border-b border-white/5">
              <div className={`flex items-center gap-2 ${theme.text}`}>
                <Newspaper size={15} />
                <span className="text-[10px] font-mono font-bold tracking-wide uppercase">BẢN TIN SÁNG 07:00</span>
              </div>
              <button 
            ...
            <div className="mt-4 space-y-2">
              ...
              <div className="flex items-center gap-3 text-[9px] text-slate-500 font-mono">
                <span className="flex items-center gap-1"><Clock size={10} /> Hôm nay, 07:00 AM</span>
              ...
              <div className="pt-4 flex flex-wrap gap-2">
                ...
                {onDiscussWithChatbot && (
                  <button
                    ...
                    className={`inline-flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-bold ${theme.bgMuted} ${theme.text} border ${theme.borderMuted} hover:bg-white/10 transition cursor-pointer`}
                  >
                    <MessageSquare size={12} /> Hỏi đáp qua Chatbot
                  </button>
                )}
              </div>
            </div>
